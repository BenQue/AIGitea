...
NewEmaint 的 `appserver-test` 只存在于本地 OrbStack DockerLab；公司侧只有上述两台 VM。其 operator
runbook、inventory/handoff/evidence schema 和所有 company/live `NOT RUN` 边界见
...
该 pilot 的 Gitea 建设采用 operator `1.1.0` 的 `greenfield-parallel-replacement`：legacy Docker Gitea
保持不变，新 systemd Gitea/PostgreSQL 仅使用固定的独立 `aisoft-gitea` namespace 与 loopback candidate
tuple；Stage 10 inventory v2 和 Stage 20 transition v1 必须先由人分阶段批准。`legacy migration/phase-out`、
DNS/TLS、反向代理、正式仓库导入与 traffic cutover 都是后续独立 Change；当前公司 Stage 10–50 全部
`NOT RUN`。逐阶段合同见 [company-delivery runbook](company-delivery/runbook.md)。
...
下表是 Windows/具有公司隔离测试环境的通用路线；不能套用到 NewEmaint pilot：
...
NewEmaint pilot 改走 [`company-delivery/runbook.md`](company-delivery/runbook.md) 的 Stage 00–110：本地
...
sync timer、Actions auto deploy 与 production gate 保持 disabled/inactive；公司侧全部阶段当前为
`NOT RUN`。
...
- NewEmaint 两台公司 VM 人工 operator workflow：[company-delivery/runbook.md](company-delivery/runbook.md)
