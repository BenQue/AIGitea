...
→ release.json + architecture.lock.json + Compose checksum
→ image@sha256 digest publish
→ RegistryTransport 或同 manifest OfflineBundleTransport
→ 独立 appserver-test deploy + migration + compose up --wait + exact health
...
- Registry transport 只按 digest pull；offline transport 在 `docker image load` 前验证
  manifest、Compose、architecture lock、inventory、archive SHA256 和 tar 路径安全。
