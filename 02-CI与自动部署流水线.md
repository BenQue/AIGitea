...
> 定位（#232，2026-09-02）：本册是交付形态参考（按项目选用），适用环境：Linux（PM2/SQLite 试点 as-built）。它不是任何项目部署步骤的事实源；平台只给环境级原则（`skill-for-codex/references/onboarding-runbook.md` §4），具体步骤、脚本、参数与环境差异由项目仓自行声明与实现。#233 判定：正文保留为参考记录；正文中「默认」「新 Linux 默认」等措辞不构成平台合同，交付形态由项目声明。
...
迁移后 `rsdesign-new` 的测试部署入口曾以应用仓库 `main` 上的 workflow 与其 AppServer 侧
profile 为准；本册 §1–§7 只保留 `gitea-ci` 时期的历史证据。该项目已随 #252 退出平台治理。
...
**有这条链路，也不等于它覆盖每一次 merge。** 只有声明 `application-deploy` 的仓库才是
「合并即部署」，声明了 `verification` 的变更在那里等 `deployed`；其余仓库（含未声明、按缺省
取 `application-deploy-selective` 的）由 `mark-completed-issues.sh` 当场写 `completed`，
如果这次 merge 后来确实被部署，本节这个 hook 会把整个生命周期维度重写成 `deployed`
（`03` §11，#192）。
