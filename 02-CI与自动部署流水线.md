...
> 定位（#232，2026-09-02）：本册是交付形态参考（按项目选用），适用环境：Linux（PM2/SQLite 试点 as-built）。它不是任何项目部署步骤的事实源；平台只给环境级原则（`skill-for-codex/references/onboarding-runbook.md` §4），具体步骤、脚本、参数与环境差异由项目仓自行声明与实现。正文原样保留，去留由后续「过时文档清理」Issue 处置。
