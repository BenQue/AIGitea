...
11. For `needs-human-decision`, set `assessed_complexity: needs-human-decision` and `contract_effect: unclear`, omit `effective_complexity`, and state the single decision needed in `reason`. Do not choose a fallback complexity or emit multiple questions.
