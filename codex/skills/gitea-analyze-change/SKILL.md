...
- `change_type`: `bugfix`, `feature`, `docs`, `test`, `refactor`, `maintenance`, `platform`, `security`, `reliability`, or `data`, corresponding to exactly one primary `type/*` label such as `type/feature`.
...
10. Use stable `risk_flags` names that directly identify every forced condition. Set semantic `required_docs` to `summary` for `small`, and initially to `summary`, `spec`, and `plan` for `complex`; the deterministic route removes spec/plan for repositories whose manifest declares `change_control: development`. Append `verification` when the acceptance evidence cannot be reproduced by diff review and required CI. Deployment and migration always qualify, and so does any change whose acceptance depends on a real-environment or one-shot observation. Appending `verification` declares that the change owes a verification record; it never declares that the change deploys.
...
12. For `needs-human-decision`, set `assessed_complexity: needs-human-decision` and `contract_effect: unclear`, omit `effective_complexity`, and state the single decision needed in `reason`. The wrapper must omit that key from both summary front matter and the `## AI 判级` YAML block; it must not leave an empty value or placeholder. Do not choose a fallback complexity or emit multiple questions.
13. Identify missing acceptance criteria that must be resolved before `approved` can start a Development Loop. Do not claim tests ran unless they actually ran.
