...
8. Keep fixed `ensure-gitea-collaborator.sh` (`ci-bot` + Write) only for already-onboarded profiles during migration. Never use it for new projects. Retire `ci-bot` one repository at a time only after the new project agent passes private read, Issue/comment/label, feature push, PR, main-push-denied, and main-merge-denied evidence.
