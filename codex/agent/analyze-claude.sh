#!/usr/bin/env bash
# Analyze one Issue, then create its validated readable change name and summary.
set -euo pipefail
...
"$SCRIPT_DIR/claude-analyzer.sh" "$ISSUE_FILE" "$RESULT_FILE" "$AGENT_REPO_DIR"
SLUG="$(python3 -m aisoft_loop.cli analysis-slug "$RESULT_FILE")"
WORKTREE="$(prepare_change_worktree "$N" true "$SLUG")"
...
SUMMARY_DIR="$WORKTREE/docs/changes/$N-$SLUG"
...
git -C "$WORKTREE" add "docs/changes/$N-$SLUG/$SUMMARY_NAME"
if ! git -C "$WORKTREE" diff --cached --quiet; then
  git -C "$WORKTREE" commit -m "docs: analyze issue #$N"
fi
git -C "$WORKTREE" push -u origin "$BRANCH"

SUMMARY_URL="$GITEA_URL/$GITEA_OWNER/$GITEA_REPO/src/branch/change%2F$N-$SLUG/docs/changes/$N-$SLUG/$SUMMARY_NAME"
python3 -m aisoft_loop.cli apply-analysis "$N" "$RESULT_FILE" "$SUMMARY_URL"
printf 'Claude analysis completed for Issue #%s on %s\n' "$N" "$BRANCH"
