"""Issue #278: standalone stdout-only diagnostics; no adoption or mutation grant."""
...
VERSION = "1.0.0"
CONTRACT = "company-platform-baseline-diagnostics/v1"
...
VALUES = {
    ...
    "server_binding": obj({key: YESNO for key in (
        "section_present", "protocol_http", "http_addr_matches_profile", "http_port_matches_profile")}),
    ...
    diagnostic = obj({"contract_version": enum(CONTRACT), "collector_version": enum(VERSION),
                      **binding_schema(), "collected_at": TIME,
                      "observations": obj({key: obj({
                          ...
                          "reason": enum("observed", "probe-unavailable"),
...
def observation(key, value=None):
    if value is None:
        return {"status": "BLOCKED", "reason": "probe-unavailable", "value": None}
    ...
    if key == "systemd":
        ...
        good = all(item == "yes" for item in value.values())
    else:
...
def seal(diagnostic, now=None):
    ...
    for key, record in diagnostic["observations"].items():
        if record != observation(key, record["value"]):
...
def server_binding(profile):
    ...
    return {"section_present": "yes" if seen else "no",
            "protocol_http": "yes" if selected.get("PROTOCOL") == "http" else "no",
...
def collect(binding, *, profile=None, invoke=run, config=server_binding, now=None):
    ...
    def command(argv, parse):
        ...
        if rc != 0 or not isinstance(text, str) or len(text.encode("utf-8")) > LIMIT:
            raise Invalid("PROBE_UNAVAILABLE")
        return parse(text)
    observations = {}
    for key, probe in (("systemd", lambda: command(SYSTEMD_COMMAND, systemd)),
                       ("server_binding", lambda: config(profile)),
                       ("ufw", lambda: command(UFW_COMMAND, ufw))):
        try:
            observations[key] = observation(key, probe())
        except (Invalid, OSError, ValueError, TimeoutError, subprocess.TimeoutExpired):
