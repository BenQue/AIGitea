...
- `approved` 表示合同已明确、允许启动 Development Loop，不表示批准合并或部署。日常会话默认只保留两个确认点：提交唯一最终 PR 前确认；merge 后完成终态核对、文档检查和 worktree/本地分支清理后确认归档。
...
- manual 最终 PR 仍只由人合并；provider、project agent、platform manager、shared bot 和 site admin 都不得成为 routine merger。routine hard gate 失败必须 fail closed，不能降级到更宽权限身份、force/scheduled merge 或自动 fallback。PR merge 不传递部署授权。
