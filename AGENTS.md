...
- Issue 是所有需求、缺陷和平台变更的追踪主键；每个 Issue 绑定 exact `change/N`、`docs/changes/N/` 和唯一最终 PR。
- 新 change 文档使用 `<role>-<short-slug>-<YYMMDD>.md`；`summary` front matter 的 `documents` 必须把 `summary`、`spec`、`plan`、`verification` 等语义角色显式映射到真实 basename。Issue #57 之前的 `00-summary.md`、`01-spec.md`、`02-plan.md`、`03-verification.md` 只作读取兼容，新 writer 不再生成，也不批量重命名历史文件。
...
- 只有 complex 变更映射的 `spec` 明确授权时，Loop 才能修改 `AGENTS.md`、Agent 行为、controller、CI/部署脚本或其他治理文件；判级或执行同一变更的运行不得修改本次运行正在遵循的 `AGENTS.md`。
...
- Agent 只能在 exact `change/N` 创建符合 plan frontier 的本地原子 commit。Controller 在复核合同、分支、提交、范围和验证后，才能 fast-forward push、创建或更新唯一最终 PR、读取 CI 并投影状态；最终 merge 只由人操作，部署需要独立授权。
