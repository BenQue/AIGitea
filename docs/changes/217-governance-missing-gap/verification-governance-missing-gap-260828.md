...
- Tested source head: `6bfe4bb`（实现/测试 commits；verification-only commit 后 source bytes 不变）
...
| governance targeted | PASS | `test_gitea_governance` 44 tests PASS；含 target/cross-project missing 404、account-before-permission、strict 200 schema、401/403/500/transport、present 404 与 apply zero-mutation negatives |
| security Python suites | PASS | governance + host-access + routine-merge：202 tests PASS |
...
| `bash codex/tests/smoke.sh` | PASS | 634 tests PASS；末行 `Codex platform static smoke checks passed.` |
...
| AC | 结论 | 证据 |
|---|---|---|
| AC-1 | PASS (source/mock) / GAP (installed) | 同次 `_check` 的 exact account 404 先形成 immutable missing evidence；target permission 404 投影 `missing`，NewEMaint 精确生成 `set-routine-merger-write`；installed bytes 未更新 |
| AC-2 | PASS | routine account 三态既有矩阵不变；target 与 cross-project call inventory 均证明 account GET 早于可兼容 permission GET；present-non-admin/site-admin 404 均抛 `ApiError` |
| AC-3 | PASS | account missing 时，HTTP 200 的 null/root/missing/extra/non-string/unknown permission 全部继续 `ContractError` |
| AC-4 | PASS | missing account 对应 permission 401/403/500/transport(0) 全部保持原 `ApiError`；只有 exact 404 被兼容；apply 默认 evidence 为空，404 继续 fail closed |
...
| AC-7 | PASS | CLI JSON keys/planned action names不变；仅内部 keyword-only `known_missing_accounts`，默认 fail-closed；无 manifest、broker、credential 或 host.access.audit 改动 |
| AC-8 | PASS (source/local) / NOT RUN (remote/live) | governance 44、security 202、四个 shell suites、full smoke 634、semantic/Controller preflight 全部 PASS；push/PR/CI/install/live/merge/deploy 均未运行 |
