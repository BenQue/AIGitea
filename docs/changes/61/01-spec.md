...
- [ ] **AC-1** strict `host-access-broker/v1` manifest 与 `gitea-governance/v1` byte-exact 交叉验证
...
- [ ] **AC-2** broker CLI 只接受 manifest project、allowlisted operation 和 operation-specific typed
...
- [ ] **AC-3** project repository/Issue/PR/Git read/write 路由到自己的 project-agent；settings/protection
...
- [ ] **AC-4** Mac project-agent credential 只从 manifest 固定的 macOS Keychain service/account 读取；
...
- [ ] **AC-5** repo-local Mac binding 工具只处理 manifest 中 exact checkout/remote，配置
...
- [ ] **AC-6** profile contract 使用 `GITEA_IDENTITY` + fixed `GITEA_TOKEN_FILE`，profile 本身不含 token。
...
- [ ] **AC-7** NewEMaint=`emaintenance`、HSDB=`hsdb`、rsdesign-new=`rsdesign`、
...
- [ ] **AC-8** profile apply 后验证 authenticated exact identity/target 和目标文件 metadata；相同输入第二次
  为 no-op，不创建新 backup。rollback 恢复 bytes/uid/gid/mode并再次 read-back，不影响其它 project。
- [ ] **AC-9** Gitea HTTP 401/403/404、transport/API/schema 错误、identity/target mismatch 在 broker 和
...
- [ ] **AC-10** installer 只复制 versioned broker/runtime/manifest/helper/profile-migration candidate；不创建
...
- [ ] **AC-11** focused tests 覆盖 operation/target allowlist、无 shell/URL/merge surface、credential
...
- [ ] **AC-12** `bash -n`、ShellCheck（若可用）、focused tests、`bash codex/tests/smoke.sh`、strict JSON、
