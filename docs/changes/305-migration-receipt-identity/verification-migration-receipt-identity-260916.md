...
- 基线：本分支最初建在 `origin/main` = `e5ed35e`（`Merge pull request 'test(#301):
  release evidence boundary 豁免 Python 字节码产物并隔离回归字节码' (#303)`）之上，
  下表中「改动前」的证据都取自那一点。PR 开出后 `main` 前进到 `803fd90`
  （`#306`，即 #304），本分支已变基到它，见下方「变基复核」。两次基线都经 broker
  `git.fetch.main` 读回确认。
...
| 变基前最终 head 全量 `bash codex/tests/smoke.sh` | PASS | 变基前的三个 head 上各跑一次，均 `Ran 965 tests ... OK` |
| 变基到 `803fd90` 后全量 `bash codex/tests/smoke.sh` | 见下方「变基复核」 | 变基后重跑 |
...
| AC-5 | PASS | `docker-release/README.md` 的 State and rollback 一节补写 receipt 以 migration identity 为键、`release_id` 仅审计、放宽只限 `completed` 一支；`06-运维手册与踩坑集.md` 新增踩坑 31（原写 30，与先合并的 #304 撞号，变基时改为 31） |
...
- PR #307 开出后 `main` 从 `e5ed35e` 前进到 `803fd90`（合并了 #304 的 PR #306），
  `merge_base` 仍停在 `e5ed35e`，`gitea.pull.read` 读回 `mergeable: false`。
  `aisoft-platform` 保留 `block_on_outdated_branch: true`（踩坑 29 的 #299 裁决），
  因此过期分支即使 CI 全绿也不能合并。
- 唯一重叠文件是 `06-运维手册与踩坑集.md`，冲突块恰好三行：#304 占用了踩坑 30，
  本变更也写了 30。按踩坑表的处理方式从 `origin/main` 重建——保留 #304 的 30，
  本变更改为 31。`diff` 对 `origin/main` 复核：该文件相对 main 只多出一行，
  即本变更的第 31 行，#304 的行原样保留。其余文件无冲突。
- 变基后本分支相对 `803fd90` 的改动集合与变基前相对 `e5ed35e` 的一致，
  唯一差异是那一个踩坑编号。
- 变基后最后一条改动源码的 commit 是 `ccc6864`（T02）。其后每一条 commit 都只改
  `docs/` 下的文字。
- 变基后全量 `bash codex/tests/smoke.sh`：PASS，`Ran 965 tests ... OK` /
  `Codex platform static smoke checks passed.`
- 变基后 `aisoft-loop check-change-documents --repo .`：PASS，`gap=0`。
- 此处不钉最终 head 的 40 位 SHA：补记这一行本身就会改变它。钉住的是
  「最后一条源码 commit」与「每个 head 都跑过全量 smoke」这两条可复核的事实。
...
- **踩坑编号是跨 PR 共享的手工常数，本次真的撞了**。本变更原写 30，#304 的 PR #306
  先合并并占用了 30，变基时改为 31。下一个并行 PR 若也写 31，同样要在变基时改号：
  这个冲突文本干净、只有一行，但**整块取一侧会静默抹掉对方刚合并的行**，
  必须从 `origin/main` 重建后用 `diff` 复核只多出自己那一行。
