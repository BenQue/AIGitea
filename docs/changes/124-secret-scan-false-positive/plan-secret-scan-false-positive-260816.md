...
|---|---|---|---|
| T04 | 增加 opaque-archive RED fixture；移除正常 builder 的 image 内容扫描；运行 exact `006d...` artifact-only → 双构建 → checksum equality → 双 verify-handoff → input revalidation/cleanup | T03 | PASS on clean `699dbbe...`；双 verify、input unchanged、cleanup PASS |
