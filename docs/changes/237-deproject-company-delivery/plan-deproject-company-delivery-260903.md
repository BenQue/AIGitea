...
| AC-2 | `find company-delivery -type f \| wc -l` = 19 与判定表逐行对照；`grep -rci NewEmaint company-delivery/`（README/runbook = 0，合计 8）；`for f in company-delivery/schema/inventory-v*.json; do jq -r '."$comment"' $f; done` 每个含「历史证据」 |
