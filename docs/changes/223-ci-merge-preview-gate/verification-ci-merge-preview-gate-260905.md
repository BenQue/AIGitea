...
- Commit SHA: `0cb078f`（T04），分支 `change/223-ci-merge-preview-gate`
- 基线：`origin/main` = `87b3aa4`（rebase 之后；开工时是 `07084de`，中途 #180 合并进 main）
...
| 本 PR 的 `CI / verify (pull_request)` | 待 PR 建立后回填 | AC-8 的运行证据 |
...
| AC-8 | 部分达成，待 CI | `.gitea/workflows/ci.yml` 的 `name: CI`、`verify:`、`on: pull_request` 三处逐字未变（diff review）；运行证据待本 PR 的 `CI / verify (pull_request)` |
...
- **AC-8 的运行证据待本 PR 的 CI**。若合并预览步骤在本平台 runner 上跑不通，
  按 spec 的回滚约束回退 `.gitea/workflows/ci.yml` 这一处 hunk，AC-8 记为未达成。
