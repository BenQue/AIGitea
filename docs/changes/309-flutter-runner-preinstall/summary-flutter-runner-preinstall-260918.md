...
- **不修改共享 CI 服务配置**（推荐形状）：平台只提供 `/opt/flutter/3.32.8/bin` 与
  `/opt/act-runner/.pub-cache` 两个固定路径并在 `01` 文档声明为平台合同，消费方 workflow
  在 job 级 `env:` 自行声明。改 `runner.envs` 并重启 act_runner 是单独的闸门 3，spec 建议拒绝。
...
该追加项的后半句越出了 Issue 原授权闸门 1 的边界，spec 因此把闸门拆成 3 条逐条授权。
闸门 3（改 `config.yaml` 并重启 act_runner）影响三个仓库的全部 CI job，spec 建议拒绝并给出
替代形状：平台只提供两个路径，消费方 workflow 自行声明 env。
...
| 闸门 3 重启 act_runner 会杀掉在跑的 job，并改变三仓全部 job 的环境 | spec 建议拒绝闸门 3；改由消费方 workflow 声明 env，平台侧零重启。若仍获批，重启前 `orbstack.runner.status` 确认 `execution.child_count == 0` |
...
| 回滚 | `rm -rf /opt/flutter/3.32.8` 与 `rm -rf /opt/act-runner/.pub-cache`（两者本次之前都不存在）；`unzip` 保留，移除它不属于本次回滚范围 |
