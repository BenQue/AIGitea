...
> 版本：v3.6 source contract ｜ 更新：2026-09-03 ｜ 状态：Issue #208 将交互会话默认确认收敛为“合同/启动确认”和“提交最终 PR 前确认”，并定义 routine small 的独立 merger/hard gate；#208 自身及所有 manual 集合仍由人合并。source 合并不等于 installed/live 启用或部署。
...
两个 adapter 等价、可互换，由 `IMPLEMENT_PROVIDER` 显式选择。Matt skills 提供完整开发编排语义，Codex 和 Claude Code 只替换模型执行器；三者都不复制或取代平台标签状态机、Git/Gitea 远端操作、测试硬门和终态判断。项目差异只存在于显式 profile、仓库内合同、`docs/agents/` tracker 配置与 versioned verifier 中。
...
- 每项目 VM profile 的 `analysis_provider` 与 `implement_provider`（`codex/config/host-access-broker.json` `projects[].vm_profile`）取值 `codex`/`claude`/`none`，定义与含义见 [04 §4](04-Agent编排与定时任务.md#4-analyzer)；`implement_provider` 迁移期固定 `none`。

## 4. 当前 provider 基础

两个 adapter 共用的 source baseline：
...
- 安装幂等、xtrace/argv token 防泄漏和两个 provider implementation parity gate 的 mock 回归。
- `project-poll.sh`、每项目 state/worktree namespace、mode 400/600 profile 校验和禁用式 `aisoft-agent@.service/.timer` 模板。
- `provider-poll.sh` 的 Claude/Codex/none 路由；`cli.py` 按 `IMPLEMENT_PROVIDER` 显式选择 adapter，禁用与未知 provider 一律失败关闭，不静默回退到另一 provider。
- Codex adapter 与 Claude adapter（`claude-provider.sh`、`claude-analyzer.sh`、`analyze-claude.sh`）都在中央 source，由 `install-vm.sh` 一并安装。
...
- adapter 输出规整：Claude 会先输出散文再给 JSON，`claude-analyzer.sh`/`claude-provider.sh` 先经 `extract-json` 取最后一个 JSON 对象再交严格校验器；共享结果校验保持严格不放宽。
- `AGENTS.md`、VM 全局指导、阶段型 skills、复合 `aisoft-platform` skill、静态 smoke 与目标目录安装脚本。

仍未完成：真实 VM 上的 Claude 一次 real Issue pilot；`IMPLEMENT_PROVIDER` 默认仍为 `none`。
历史试点与安装证据见 [archive/平台状态历史-20260902.md](archive/平台状态历史-20260902.md)。
...
仓库初始化先完成 Gitea profile、protected `main`、required CI、项目 Agent 权限与 27-label read-back，再运行 `codex/install-skills.sh <target-home>` 安装已验证 snapshot；Claude 侧等价入口是 `skill-for-claude/install.sh <target-home>`（装到 `~/.claude/skills/`，两侧共用 `skill-for-codex/references/`），改动源后用 `skill-for-claude/check-drift.sh` 核对回到 `CLEAN`。两个 installer 都经 `codex/lib/install-source-guard.sh` 拒绝陈旧源。首次进入项目时显式调用 `$setup-matt-pocock-skills`，tracker 选择 `Other`，并把 `templates/docs/agents/` 三个模板安装为 `docs/agents/issue-tracker.md`、`triage-labels.md`、`domain.md`。若 setup 要修改本次运行正在遵守的 `AGENTS.md`，本轮只提交 proposal，下一次独立授权再应用。
...
9. **生产负向边界**：provider 无权部署生产，只能准备经非生产验证的修复 PR。
...
这些 classifier case 由 `codex/runtime/tests`（`test_classification.py` 等）固化为合同测试。
...
- 默认 `IMPLEMENT_PROVIDER=none`；启用是每项目独立验收门。
- 试点失败时停止新 controller，保留 analyzer，回到 Mac 人机交互开发。
- 任一 provider 不可用时不自动切换到另一 provider 执行同一 active Issue；先结束或转移状态，再显式选择 provider。
- 删除任一 provider 的认证、脚本或 skills 须另行批准；两者配置独立保存（§3）。
